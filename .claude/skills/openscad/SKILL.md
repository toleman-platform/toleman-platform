...
On Linux, install via `apt-get install openscad` instead. In headless/sandboxed
containers without a GPU or X server, rendering tools (`preview.sh`,
`multi-preview.sh`, `render-with-params.sh` for PNG output) will segfault with
"Unable to open a connection to the X server" unless run under a virtual
display, e.g. `apt-get install xvfb` and prefix commands with `xvfb-run -a`.
STL export (`export-stl.sh`) does not need a display and works without xvfb.
