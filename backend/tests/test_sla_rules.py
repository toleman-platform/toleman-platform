...
def test_violation_computed_against_real_first_seen(engine):
    ...
    old_finding_id = _make_finding(
        engine, target_id, severity=Severity.HIGH, first_seen=utcnow() - timedelta(days=10)
    )
    recent_finding_id = _make_finding(
        engine, target_id, severity=Severity.HIGH, first_seen=utcnow() - timedelta(days=1)
    )
    with Session(engine) as session:
...
def test_closed_finding_never_in_violation(engine):
    ...
    finding_id = _make_finding(
        ...
        first_seen=utcnow() - timedelta(days=100),
...
def test_findings_endpoint_reports_sla_fields(client, engine):
    ...
    _make_rule(engine, ws_id, None, Severity.CRITICAL, 1)
    finding_id = _make_finding(
        engine, target_id, severity=Severity.CRITICAL, first_seen=utcnow() - timedelta(days=5)
    )
...
def test_dashboard_sla_compliance(client, engine):
    ...
    _make_finding(engine, target_id, severity=Severity.CRITICAL, first_seen=utcnow() - timedelta(days=5))  # violated
    _make_finding(engine, target_id, severity=Severity.LOW, first_seen=utcnow() - timedelta(days=5))  # compliant
    _make_finding(engine, target_id, severity=Severity.MEDIUM, first_seen=utcnow() - timedelta(days=5))  # no rule, excluded
