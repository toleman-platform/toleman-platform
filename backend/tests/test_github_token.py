...
def test_resolve_lazily_purges_expired_token(session):
    ws = _workspace(session)
    _store_token(session, ws.id, plaintext="ghp_expired", expires_at=datetime.now(UTC).replace(tzinfo=None) - timedelta(hours=1))
...
def test_resolve_returns_unexpired_token(session):
    ws = _workspace(session)
    _store_token(session, ws.id, plaintext="ghp_future", expires_at=datetime.now(UTC).replace(tzinfo=None) + timedelta(hours=1))
...
def test_purge_expired_tokens(session):
    ...
    _store_token(session, ws1.id, plaintext="ghp_expired", expires_at=datetime.now(UTC).replace(tzinfo=None) - timedelta(hours=1))
    _store_token(session, ws2.id, plaintext="ghp_future", expires_at=datetime.now(UTC).replace(tzinfo=None) + timedelta(hours=1))
