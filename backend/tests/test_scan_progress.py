...
def _completed_scan(session, target_id: int, tool: str, seconds: int, age_minutes: int = 0):
    started = datetime.now(UTC).replace(tzinfo=None) - timedelta(minutes=age_minutes, seconds=seconds)
    scan = Scan(
        target_id=target_id,
        tool=tool,
        branch="main",
...
def test_failed_runs_are_not_sampled(engine):
    ...
    _, target_id = _make_workspace_and_target(engine)
    with Session(engine) as session:
        started = datetime.now(UTC).replace(tzinfo=None) - timedelta(seconds=900)
        for _ in range(5):
            session.add(
                Scan(
                    target_id=target_id,
...
def test_zero_and_negative_durations_are_discarded(engine):
    ...
    _, target_id = _make_workspace_and_target(engine)
    with Session(engine) as session:
        started = datetime.now(UTC).replace(tzinfo=None) - timedelta(seconds=60)
        for delta in (0, -5, -10):
            session.add(
                Scan(
                    target_id=target_id,
...
def test_elapsed_counts_up_while_running(engine):
    ...
    scan = Scan(
        ...
        branch="main",
        status="running",
        started_at=datetime.now(UTC).replace(tzinfo=None) - timedelta(seconds=45),
    )
...
def test_elapsed_freezes_at_completion(engine):
    _, target_id = _make_workspace_and_target(engine)
    started = datetime.now(UTC).replace(tzinfo=None) - timedelta(seconds=300)
    scan = Scan(
        target_id=target_id,
        tool="semgrep",
        branch="main",
...
def test_get_scan_reports_elapsed_and_eta_while_running(engine, client):
    ...
    with Session(engine) as session:
        ...
        running = Scan(
            ...
            branch="main",
            status="running",
            started_at=datetime.now(UTC).replace(tzinfo=None) - timedelta(seconds=10),
        )
        session.add(running)
        session.commit()
